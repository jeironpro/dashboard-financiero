...
export function formatNumber(value: number): string {
  return new Intl.NumberFormat('es-MX', { useGrouping: true, maximumFractionDigits: 0 }).format(value)
}
...
export function formatMonthKey(monthKey: string): string {
  ...
  const label = new Intl.DateTimeFormat('es-MX', { month: 'short' }).format(new Date(year, month - 1, 1))
