        ...
        return (
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className={cn('font-display text-lg font-semibold tracking-tight', meta.tint)}>
                {plan.name}
              </h3>
            ...
            <div className="mt-4 border-t border-rule pt-3 text-xs text-faint">
              <span className="num font-semibold text-ink">{subs.length}</span> suscripciones ·{' '}
              <span className="num font-semibold text-ink">{formatMoney(planMrr)}</span> MRR
            </div>
          </div>
        )
      })}
