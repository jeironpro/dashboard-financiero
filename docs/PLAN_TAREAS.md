# Plan de implementación — Dashboard Financiero «Suma»
...
| 2 | `feature/design-tokens` | Sistema de diseño Hallmark (Hum) | `tokens.css` (papel crema, multi-acento pear/cyan/coral, mint/lavanda) + `src/index.css` (mapeo a variables shadcn) + fuentes Plus Jakarta Sans / JetBrains Mono | Sin hex sueltos: todo color/fuente vía token; contraste AA; stamp Hallmark al inicio de `tokens.css` |
| 3 | `feature/mock-data` | Datos MOCK realistas | `src/data/mock-data.json` + `src/types/finance.ts` + `src/data/index.ts` (deriva KPIs) + `src/lib/format.ts` (es-MX, MXN) + `src/lib/status.ts` | Sumas internamente consistentes (MRR = Σ suscripciones activas; ingresos = Σ pagos exitosos; IVA 16 %); tipos sin `any` |
...
- **Tema**: Hum (look del ejemplo Tally de la skill Hallmark) — papel crema, acento primario pear, secundario cyan, pop coral, mint para éxito; Plus Jakarta Sans (display/cuerpo) + JetBrains Mono (labels/tabulares). Sin serif, sin blanco puro, sin negro puro, sin esquinas cuadradas.
